...
function looks_like_FIDE ($chaine)
{
  // retourne vrai si la chaine a la tronche d'une licence, à savoir longueur de 8 caractères, et tous des chiffres
  $chlen = strlen($chaine);
  $is_fide = ($chlen == 8 OR $chlen == 9); 
  if ($is_fide) {
    $ch_arr = str_split($chaine);
    $is_fide = true;
    foreach ($ch_arr as $char) {
      $is_fide &= preg_match('/^[0-9]$/', $char);
    }
  }
  return $is_fide;
}
...
function item_array_to_player_array ($items_array)
{
    ...
    if (! is_null($custfielsobj))
    {
      ...
      
      {
        ...
        if ($is_licence_field)
        {
          ...
          if (looks_like_licence($answer) OR looks_like_FIDE($answer)) $candidate = $answer;
        }
        echo $customField->name . " | " . stripos($customField->name, 'tournoi') . "\n";
        if (!(stripos($customField->name, 'tournoi') == false) ) $player["subevent"] = $customField->answer;
      }
  ...
  echo "sortie de item_array_to_player_array\n\n";
  return $player_arr;
}
